...
class IntentDiagram extends Component {
    ...
    toggleStar = (intentQuestion) => {
        // store.dispatch(toggleBookmark({question: intentQuestion, bookmarkType: "star", currentState: this.isStarred}))
        this.isStarred = !this.isStarred;
        console.log(this.isStarred);
    }

    toggleFlag = (intentQuestion) => {
        // store.dispatch(toggleBookmark({question: intentQuestion, bookmarkType: "flag", currentState: this.isFlagged}))
        ...
        return (
            <div className="text-black font-cabin flex flex-col place-self-center pt-1 -mb-9">
                <div className="inline-flex place-items-center max-w-[47rem]">
                    <button className="w-16 h-16" onClick={this.toggleStarred} aria-label="star button">
                        <label>
                            <svg
                            className={this.props.isStarred ? "w-16 m-3 fill-yellow cursor-pointer": "w-16 fill-transparent cursor-pointer"}
                    ...
                    <h3
                        className="rounded-lg bg-off-white self-center p-5 shadow-lg shadow-blue/10 break-words text-center md:text-xl 2xl:text-2xl min-w-[30rem] max-w-[35rem] md:max-h-[10.5rem] 2xl:max-h-[15rem]"
                        data-testid={this.props.question}
                        tabIndex={0}
                    >
                        {this.props.question}
                    </h3>
                    <button className="w-16 h-16" onClick={this.toggleFlagged} aria-label="flag-button">
                        <label>
                            <svg
